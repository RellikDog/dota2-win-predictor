...
def get_names(df, heroes=heroes):
    ...
    for indx, hero_num in enumerate(hero_names):
        for hero in heroes:
            if hero['id'] == hero_num:
                hero_names[indx] = hero['name']
    df.columns = list(df.columns[:4]) + list(hero_names)
    return

def get_nums(df, heroes=heroes):
    hero_nums = [i for i in df.columns[4:]]
    for indx, hero_name in enumerate(hero_nums):
        for hero in heroes:
